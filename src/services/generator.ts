...
import { circuitBreakerRegistry, RetryManager } from '../utils/circuitBreaker';
import { measureExecutionTime, structuredLogger } from '../utils/logger';
import { GracefulDegradationManager, ServiceHealth, FallbackStrategy } from './gracefulDegradation';
import { cacheManager, CacheStrategy, CacheTier } from './intelligentCache';
import { requestDeduplicationService, deduplicate } from './requestDeduplication';
...
const workoutCache = cacheManager.getCache('workout-generation', {
 ...
 */
function generateCacheKey(promptData: { prompt: string; variant: any }, options: GenerationOptions): string {
  const keyData = {
...
export async function generateWorkoutWithOptimization(
...
export async function generateWorkout(
  ...
  try {
    ...
    return await degradationManager.executeWithDegradation(
      'openai',
      async () => {
...
Output must be valid JSON matching the provided schema exactly. Focus on progressive overload, movement quality, and sustainable training practices.`
        ...
        return await retryManager.execute(
          async () => {
            ...
            structuredLogger.logExternalServiceCall(
              ...
              Date.now() - startTime,
              true,
              { model: env.OPENAI_MODEL, requestId }
            );
    ...
    if (error.name === 'GracefulDegradationError') {
      throw new WorkoutGenerationError(
        'AI service is temporarily unavailable. Using fallback response.',
        'SERVICE_DEGRADED',
        error
      );
    }
    ...
    if (error.name === 'RetryExhaustedError') {
      throw new WorkoutGenerationError(
        'AI service failed after multiple attempts. Please try again later.',
        'RETRY_EXHAUSTED',
        error
      );
    }
    ...
    if (error.name === 'CircuitBreakerError') {
      throw new WorkoutGenerationError(
        'AI service is temporarily unavailable due to high failure rate.',
