...
function getExperienceDescription(experience) {
    ...
    return descriptions[experience] || descriptions.beginner;
}
function getEnergyDescription(energyLevel) {
    const descriptions = {
    ...
    return descriptions[energyLevel] || descriptions[3];
}
function getHeightWeight(profile) {
...
function getMovementPatterns(workoutType) {
...
function getPeriodizationStrategy(experience, userHistory) {
...
function getFatigueManagementStrategy(experience, energyLevel) {
    const baseStrategy = {
    ...
    let strategy = baseStrategy[experience] || baseStrategy.beginner;
...
const getRestTimeGuidelines = (experience) => {
    const guidelines = {
        beginner: {
...
const getIntensityGuidelines = (experience, workoutType) => {
    const isStrengthFocused = ['chest', 'back', 'legs', 'shoulders', 'push', 'pull'].some(type => workoutType.toLowerCase().includes(type));
    const isCardioFocused = ['cardio', 'hiit', 'conditioning'].some(type => workoutType.toLowerCase().includes(type));
...
async function getAdvancedWorkoutHistory(userId, workoutType) {
    try {
        ...
        const recentWorkouts = allWorkouts.slice(0, 10).map((workout) => {
        ...
        if (recentCompletions > olderCompletions)
            consistencyTrend = 'improving';
        else if (recentCompletions < olderCompletions)
...
async function getRecentWorkoutHistory(userId, workoutType) {
    const analysis = await getAdvancedWorkoutHistory(userId, workoutType);
    const allWorkouts = await WorkoutPlan_1.WorkoutPlanModel.find({ userId }, { sort: { createdAt: -1 }, limit: 5 });
...
async function getUserWorkoutHistory(userId) {
    try {
        const allWorkouts = await WorkoutPlan_1.WorkoutPlanModel.find({ userId });
        ...
        const workoutTypes = allWorkouts.reduce((acc, workout) => {
...
function generateProgressiveOverloadGuidance(workoutHistory, experience) {
...
function getExpertPersonaContext(profile, workoutType, historyAnalysis) {
    ...
    if (historyAnalysis.adaptationSignals.needsDeload)
        focusAreas.push('recovery_focus');
    ...
    if (historyAnalysis.recentPerformance.struggledExercises.length > 0)
        focusAreas.push('technique_refinement');
    return { specialization, coachingStyle, focusAreas };
}
// Generate AI-optimized personalized insights
async function getAdvancedPersonalizedInsights(userId, workoutType, historyAnalysis) {
    try {
...
function generateExpertPersonaIntro(expertPersona, experience) {
    const personas = {
        strength: {
...
function generateAdaptiveProgrammingContext(historyAnalysis, experience) {
    const signals = [];
    if (historyAnalysis.adaptationSignals.needsProgression) {
...
async function getPersonalizedInsights(userId, workoutType) {
...
async function buildWorkoutPrompt(userId, pre) {
    // Get the appropriate prompt variant for this user (A/B testing)
    const variant = promptVersioning_1.promptVersioning.getVariantForUser(userId);
    let profile = await Profile_1.ProfileModel.findOne({ userId });
    ...
    const promptOptimizations = await promptOptimizer_1.promptOptimizer.optimizePromptForUser(userId, workoutTypeFormatted);
    const optimizationInsights = promptOptimizer_1.promptOptimizer.generateOptimizationInsights(promptOptimizations);
    ...
    const finalPrompt = promptVersioning_1.promptVersioning.applyVariantToPrompt(basePrompt, variant);
    return {
        prompt: finalPrompt,
        variant: variant
    };
...
function getTargetMuscles(workoutType) {
    const type = workoutType.toLowerCase();
    const muscleMap = {
...
function getEnergySystemFocus(workoutType) {
    const type = workoutType.toLowerCase();
...
function getSetsRepsGuidelines(experience, goals, workoutType) {
    const type = workoutType.toLowerCase();
    ...
    const exp = experienceGuidelines[experience] || experienceGuidelines.beginner;
...
function getWorkoutProgrammingExamples(workoutType, experience) {
    const type = workoutType.toLowerCase();
    const examples = [];
...
function getRestSeconds(restRange) {
